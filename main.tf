...
resource "aws_subnet" "public" {
  ...
  tags = { Name = "techcorp-public-subnet-${tonumber(each.key) + 1}" }
}
...
resource "aws_subnet" "private" {
  ...
  tags = { Name = "techcorp-private-subnet-${tonumber(each.key) + 1}" }
}
...
# ---------------------
# Private Route Tables (each private subnet uses NAT in same AZ)
# ---------------------
...
resource "aws_instance" "web" {
  ...
  tags = { Name = "techcorp-web-${each.key}" }
}
...
resource "aws_instance" "db" {
  ...
  key_name                    = var.key_name == "" ? null : var.key_name
  user_data                   = templatefile("${path.module}/user_data/db_server_setup.sh", { db_password = var.db_password })
